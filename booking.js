...
document.addEventListener("DOMContentLoaded", function () {
  ...
  initImageUpload();
  initRequestOptions();
  initPropertySizeUnit();

  // Modify the existing updateBookingSummary function by adding this at the end
  const originalUpdateBookingSummary = updateBookingSummary;
  updateBookingSummary = function () {
    // Call the original function first
    originalUpdateBookingSummary();

    // Add our custom summary updates
    updateRequestOptionsInSummary();
    updatePropertySizeInSummary();
  };
});
...
function validateStep(stepNumber) {
  ...
  if (stepNumber === 1) {
    ...
    const selectedService = document.querySelector(
      ".service-category .service-option.selected"
    );
    if (!selectedService) {
    ...
    const specificService = document.querySelector(
      ".specific-services .service-option.selected"
    );
    if (!specificService) {
  ...
  // Validation for Step 2: Date and Time
    ...
    const serviceFrequency = document.querySelector(
      'input[name="service-frequency"]:checked'
    );
    if (!serviceFrequency) {
...
function initServiceSelection() {
  ...
  const categoryOptions = document.querySelectorAll(
    ".service-category .service-option"
  );
  ...
  categoryOptions.forEach((option) => {
    option.addEventListener("click", function () {
      ...
      bookingData.serviceCategory =
        this.querySelector(".service-name").textContent;
      saveBookingData(bookingData);
    });
  });

  // Specific service selection
  const specificOptions = document.querySelectorAll(
    ".specific-services .service-option"
  );
...
function updateBookingSummary() {
  ...
  updateSummaryField(
    "summary-property-size",
    `${bookingData.propertySize} sq ft`
  );
...
/**
 * Initialize the property image upload functionality
 */
function initImageUpload() {
  ...
  if (!uploadArea || !fileInput || !uploadedImagesContainer) return;
  ...
  function handleFiles(files) {
    ...
    if (remainingSlots <= 0) {
      alert(
        "You can upload a maximum of 3 images. Please remove some images first."
      );
      return;
    }
    ...
    for (let i = 0; i < filesToProcess; i++) {
      ...
      reader.onload = function (e) {
        ...
        removeButton.addEventListener("click", function (e) {
          ...
          const bookingData = getBookingData();
          if (!bookingData.propertyImages) {
            bookingData.propertyImages = [];
          }

          // Remove this image from the stored data
          const imageIndex = Array.from(
            uploadedImagesContainer.children
          ).indexOf(imageContainer);
          if (imageIndex !== -1) {
            bookingData.propertyImages.splice(imageIndex, 1);
            saveBookingData(bookingData);
          }
        });
        ...
        // Store image data in booking data
        const bookingData = getBookingData();
        if (!bookingData.propertyImages) {
          bookingData.propertyImages = [];
        }

        bookingData.propertyImages.push(e.target.result);
        saveBookingData(bookingData);
      };
...
function initRequestOptions() {
  ...
  if (requestEstimate) {
    requestEstimate.addEventListener("change", function () {
      const bookingData = getBookingData();
      bookingData.requestEstimate = this.checked;
      saveBookingData(bookingData);
    });
  }

  if (requestCall) {
    requestCall.addEventListener("change", function () {
      const bookingData = getBookingData();
      bookingData.requestCall = this.checked;
      saveBookingData(bookingData);
    });
  }
}
...
/**
 * Update the booking summary with request options
 */
function updateRequestOptionsInSummary() {
  const bookingData = getBookingData();

  // Create or update request options in summary
  let requestsHtml = "";
  if (bookingData.requestEstimate) {
    requestsHtml += '<div class="summary-badge">Free Estimate Requested</div>';
  }

  if (bookingData.requestCall) {
    requestsHtml += '<div class="summary-badge">Phone Call Requested</div>';
  }

  // Only show if we have some requests
  if (requestsHtml) {
    let requestSummary = document.querySelector(".summary-requests");

    if (!requestSummary) {
      // Create the container if it doesn't exist
      requestSummary = document.createElement("div");
      requestSummary.className = "summary-section summary-requests";

      const title = document.createElement("h4");
      title.className = "summary-section-title";
      title.textContent = "Special Requests";

      const content = document.createElement("div");
      content.className = "summary-badges";

      requestSummary.appendChild(title);
      requestSummary.appendChild(content);

      // Insert after customer info
      const customerInfo = document.querySelector(
        ".summary-section:nth-child(2)"
      );
      if (customerInfo && customerInfo.nextElementSibling) {
        customerInfo.parentNode.insertBefore(
          requestSummary,
          customerInfo.nextElementSibling
        );
      } else {
        document.querySelector(".booking-summary").appendChild(requestSummary);
      }
    }

    // Update the content
    requestSummary.querySelector(".summary-badges").innerHTML = requestsHtml;
  }
}

// Update the property size unit
function initPropertySizeUnit() {
  const propertySizeUnit = document.getElementById("property-size-unit");

  if (propertySizeUnit) {
    propertySizeUnit.addEventListener("change", function () {
      const bookingData = getBookingData();
      bookingData.propertySizeUnit = this.value;
      saveBookingData(bookingData);
    });
  }
}

// Update the format for property size in the summary
function updatePropertySizeInSummary() {
  const bookingData = getBookingData();

  if (bookingData.propertySize && bookingData.propertySizeUnit) {
    const unitLabel =
      {
        sqft: "sq ft",
        sqm: "sq m",
        acres: "acres",
        hectares: "hectares",
      }[bookingData.propertySizeUnit] || "sq ft";

    updateSummaryField(
      "summary-property-size",
      `${bookingData.propertySize} ${unitLabel}`
    );
  }
}
